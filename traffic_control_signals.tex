...
\hypersetup{unicode=true,
  ...
  pdfcopyright={Copyright {\copyright} 2026 by John Sauter},
...
from the east on lane B and can either cross the bridge, exiting on lane 2,
...
The duration of the Passage timer can vary.  If a long gap is not found
quickly the finite state machine will settle for a shorter gap.
...
anyway.  This will allow conflicting traffic to proceed even when it needs
...
If the signal face has been red for Red Limit seconds the finite state
...
Unlimited means
that the timer never completes; there is no limit on
how long either signal face can remain red.
...
When the other finite state machine sees that its Clearance Requested
toggle has been set, it starts the process which will lead to
it turning its signal face red.
...
If the other finite state machine is waiting for this lane to be clear
before turning green, this causes the Conflicting Paths are Clear toggle
to be set in the other finite state machine.
...
Each finite state machine is wired to the lamps in the
signal face that it controls.
Table \ref{lamp_map_01} shows wiring between the
...
Yellow state either when there is a gap in the traffic, or if too much time
...
Here is the detailed log.  In the Time column, blank means the same time
as the row above.
...
]{{\includegraphics[width=\textwidth]{first_frame_01}}}{VPlayer.swf}
...
on lane A or lane B.  Vehicles on lane A can make a U turn (exit on lane 1),
or make a left turn (lane 4).  Vehicles on lane B can go straight through
(lane 3) or turn right (lane 2).  Vehicles entering the intersection from
the West, North, and East have the equivalent choices for a total of 16
travel paths through the intersection.

\subsection{Conflicts}

Every travel path conflicts with the cross-traffic and with traffic in
the oncoming left turn lane.  However, the left turns will allow
traffic to proceed if it is safe, a procedure known as permissive
left turns.  However, if the oncoming traffic is heavy we will
stop it to let the left turn traffic through, a procedure known as
protected left turns.

\subsection{Signal Faces}

We use signal faces just like the ones used for the single-lane
bridge for lanes B, D, F, and H.  For the left turn lanes, A, C, E,
and G we use a signal face with an additional lamp to signal whether
left turns are permissive or protected.

\subsubsection{Permissive Left Turn Signal Face}

Lanes A, C, E, and G allow both permissive and protected left turns but not
...
This intersection also has an emergency vehicle detector
which can sometimes tell
...
as described in subsection \ref{subsection:wiring_sensors_02}.

\subsection{Timers}

In addition to the timers we described for the single-lane bridge,
we need additional timers to handle permissive left turns and a timer
to handle the case of traffic that vanishes.

\subsubsection{Left Flashing Yellow Waiting Time}

Lanes A, C, E, and G can allow a left turn in the presence of oncoming traffic,
...
turn, and the signal face will show a left green arrow.
...
the signal face red even if there is no conflicting traffic and there is
still traffic turning left.

\subsubsection{Traffic Waiting}

If the Traffic Approaching or Traffic Present sensors are triggered
when the signal face is red, but a vehicle does not appear at the
intersection, there is no need to stop the cross traffic.
This can happen if a vehicle turns right or enters a driveway between
the sensor and the intersection.  This can also happen if a turning
vehicle runs over the sensor in an adjacent lane.

If, after triggering a sensor, a vehicle is not holding down the
Traffic Present sensor after this many seconds, stop asking for
permission to turn green.

\subsection{Timer Durations}

Here are the lengths of time that each timer runs.
Notice that when the finite state machine is looking to turn its signal
face red to allow conflicting traffic to flow, if the Maximum Green
timer is close to completion it shortens the Passage timer in the hope
of finding a short gap after not finding a long gap.
...
we need toggles to handle the emergency vehicle sensor and permissive
left turns.

\subsubsection{Preempt Green}

This toggle is set by the emergency vehicle detector if the emergency
vehicle is coming from the lane controlled by this finite state machine.
The purpose is to clear the lane so that the emergency vehicle will
not be blocked.
...
This can only happen for lanes A, C, E, and G, since only those
finite state machines have a partial conflicts table
that is different from their conflicts table.
...
\subsection{Wiring the Finite State Machines to the Sensors}
\label{subsection:wiring_sensors_02}
...
With eight signal faces we have eight finite state machines which
...
to the specified state as quickly as possible consistent with
safety, and keep it there until the sensor ceases to be triggered.

\subsubsection{System Programs}

The green request granted system program must now handle requests
from eight lanes, making sure that no lane gets frozen out because
two other lanes are enchanging green between them.  In addition,
the system programs for conflicting lanes must handle permissive
versus protected left turns.
...
line from each direction.  Some of the cars will be turning left
or making U turns, whereas others will be going straight through
or making right turns.  Right turn on red is allowed.

\noindent\includemedia[width=\textwidth,
  addresource=four_corners_many_animation_02.mkv,
  activate=onclick, attachfiles,
  flashvars={source=four_corners_many_animation_02.mkv &loop=false}
]{{\includegraphics[width=\textwidth]{first_frame_02}}}{VPlayer.swf}

Notice that traffic flows smoothly, with no vehicle having to wait
very long for its turn.
...
\begin{longtable}{l | l | p{8cm}}
  ...
  A & 6, 2, 1 & D6, D1, D2, F2, G1, G6, H6, H5, H4, H3, J1, pswpse, psepsw \\
  ...
  H4, H5, H6, J1, J2, pnwpne, pnepnw, pswpse, psepsw \\
  E & 5, 4, 3 & B5, C4, D2, D1, D6, D4, H5, H4, pnwpne, pnepnw  \\
  F & 2 & A2, A1, A6, D1, D2, D6, H4, H4, H3, J2, pnwpne, pnepnw, pswpse,
  psepsw \\
  G & 6, 1 & A6, A1, A2, D2, D1, D6, H6, H5, H4, H3, J1, pnwpne, pnepnw,
  pswpse, psepsw \\
  H & 6, 5, 4 & A6, B5, C4, D4, D6, D1, D2, E3, E5, E4, F2, G1, pnwpne,
  pnepnw \\
...
we can simplify the travel path conflicts table into a lane
conflicts table.  If any travel path from a first lane conflicts with
...
\begin{longtable}{c | l}
  \caption{Conflicts Table} \\
  lane & conflicts with lane \\
  \hline \endfirsthead
  \caption{Conflicts Table continued} \\
  lane & conflicts with lane \\
  \hline \endhead
  A & pse, psw, D, F, G, H, J \\
...
and lane E which omits lanes B and C.  The Partial Conflicts table
is used for permissive left turns.

\subsection{Signal Faces}

We need some additional signal faces for this intersection.  In addition
to the signal faces used for the one-way bridge and the four-corners
...
\begin{figure}[H]
  \fcapside[\FBwidth]
           ...
           {\caption{steady right arrow red, yellow, and green}}
\end{figure}
...
It is customary for parallel lanes to always show the same colors
in their signal faces.  To accomplish this, the sensors for lanes
B and C are connected to the finite state machines for both lane
B and lane C, and similarly for lanes F and G.  In addition,
their timers are the same so they always show the same color.

Similarly, when pedestrians are permitted to cross the street in one
direction, it is customary to permit pedestrians to cross in the
opposite direction, so the pedestrian sensors are wired to both their
own finite state machine and also the finite state machine for
crossing in the opposite direction.

\subsection{Timers}

We need the following additional timers to prevent traffic from
being stopped unnicessarily.
...
Here are the lengths of times that the timers run for.
Notice that the north and south lanes will shrink their Passage timers
if they have been waiting a long time for a gap.
...
When a malfunction or a traffic control officer sets
the sensor corresponding to this toggle
...
since flashing the lower lamp means that turning left is permitted.

\subsubsection{Manual Red}
When a traffic control operator sets
the sensor corresponding to this toggle the finite state machine
...
connected to a lamp in its signal face which shows a Steady Left Arrow Red.
...
Each finite state machine is wired to the sensors for its lane.
...
for lanes psw and pse to both finite state machines psw and pse,
...
machines psw and pse and also in pnw and pne.
...
\item[Green]
  ...
  When the signal face turns green
  we keep it green long enough
  for the traffic to get moving.  Afterwards if another lane
  wants this lane to turn red we wait for a break in the traffic.
...
\item [Green Request Granted]
  ...
  To prevent two lanes from bouncing green time between
  themselves, locking out a third lane,
  ...
  lanes that are requesting green,
  a list of lanes that are allowed to turn green
  and a list of lanes that have turned green since the
  currently waiting lane has had its chance.
  All of these lists start empty.

  Go through all of the lanes
  looking for those with toggle Request
  Green true.  Add each such lane to the list of
  lanes that are requesting green,
  unless the lane is already on
  the list or is in the list of lanes allowed to turn green.

  If the list of laness allowed to turn green is empty, move the
  oldest lane on the list of lanes
  that are requesting green to the list of lanes allowed to
  turn green and empty the list
  of lanes that have turned green since the currently waiting
  lane has had its chance.

  Although we wish to allow the lane that has been on the
  lanes requesting to turn green list the longest to turn green
  as soon as possible, we can let other lanes turn green
  at the same time or sooner if they will not delay this lane
  from turning green.

  If the oldest lane on the list of lanes
  that are requesting to turn green does not conflict with any of the
  lanes that are already allowed to turn green,
  move it from the list of lanes that are requesting
  to turn green to the list of lanes that are allowed
  to turn green and empty the list of lanes that have
  turned green since the currently waiting lane has had
  its chance.

  Keep doing the above until the list is empty or the oldest lane
  on the list cannot move because of a conflict.

  Now we try to let some lanes turn green out of order, being
  careful not to allow any other lane to be starved out of any
  opportunity to turn green.

  Provided the oldest lane on the list of lanes
  requesting to turn green has not been waiting too long,
  go through the list of lanes that are requesting
  to turn green starting with the oldest.
  For each such lane that does not
  conflict with any of the lanes already on the list of
  lanes allowed to turn green, and is not in the list of
  lanes that have turned green since the currently waiting
  lane has had its chance, move that lane
  to the list of lanes allowed to turn green and add it to
  the list of lanes that have
  turned green since the currently waiting lane has had its
  chance.

  Go through the list of lanes allowed to turn green.  For any
  for which toggle Traffic Flowing in its finite state machine is true,
  remove them from the list since they have now turned green.

  Go through the list of lanes allowed to turn green.  For each
...
\item[Clearance Requested]
  ...
  When a lane wants to turn green,
  ...
  that conflict with the lane requesting to turn green.
  ...
  consult the conflicts table to see which lanes conflict with this one.
  For each such lane, set the Clearance Requested toggle in its finite
  state machine.

\item[Partial Clearance Requested]

  When a lane wants to turn green or flashing left arrow yellow,
  it must wait for some conflicting
  travel paths to become clear.
  This system program conveys the request
  for clearance to the finite state machines which control some of the
  entry lanes
  that conflict with the lane requesting to turn green.

  Go through all of the finite state machines looking for those with toggle
  Request Partial Clearance true.  For each such finite state machine,
  consult the partial conflicts table to see which lanes conflict
  with this one.  For each such lane, set the Clearance Requested toggle
  in its finite state machine.

  This is the same as Clearance Requested, but we use the partial
  conflicts table, which matches the conflicts table except
  ...
  If they cannot the finite state machine for the left turn lane
  will use its Request Clearance
...
\item[Conflicting Paths are Clear]
  ...
  Request Clearance or toggle Request Partial Clearance
  true.  For each such finite state machine, consult the conflicts table
  ...
  lane.  If all of the corresponding finite state machines have their
  Cleared toggles true, set the Conflicting Paths are Clear toggle
  in this finite state machine.

\item[Partial Conflicting Paths are Clear]

  Go through the finite state machines looking for those with toggle
  Request Partial Clearance true.
  For each such finite state machine, consult the partial
  conflicts table to determine which entry lanes contain a travel path
  that conflicts with any of the travel paths controlled by this finite state
  machine's lane.  If all of the corresponding finite state machines
  have their Clear toggles true, set the Partial Conflicting Paths are Clear
  toggle in this finite state machine.
...
\item[Safety Check]
  ...
  combination of states, such that conflicting traffic flows are simultaneously
  permitted in the
...
complete they will transition to substate Travel Path is Clear
...
]{{\includegraphics[width=\textwidth]{first_frame_03}}}{VPlayer.swf}
...
The signal faces for lanes A and E turn left flashing yellow the moment
their cars arrive since lanes B, C, F, and G are allowing traffic to flow.

The arrival of a pedestrian at psw causes lanes B, C, F, and G to turn
yellow in anticipation of turning red, but before they do the car in lane A
makes its permissive left turn.  The car in lane E, even though it arrived
after the pedestrian in psw, is allowed to make its left turn because
it does not pass through the southern pedestrian crosswalk.

Lane A turns red in order to clear the southern crosswalk, and the pedestrian
at psw can begin to cross.

The arrival of a pedestrian in lane pne causes lane E to turn red, and with
all of the vehicle lanes now red the pedestrian in lane pne can begin
to cross.

When the pedestrian in the south crosswalk has crossed, lane J turns green.
Similarly, when the pedestrian in the north crosswalk has crossed,
lane H turns green.

When lanes H and J have turned red, lane D
can finally turn green.  When it turns red lanes
B, C, F, and G turn green, getting us back to the idle state.

To summarize, if traffic arrives in quick succession on lanes
...
controlled by the operator, who will usually be a traffic control officer.
The operator has
...
In this mode the operator is expected to manually turn each signal face
...
With the mode switch set to Flashing, the Flash sensor in each finite state
machine is set, causing each finite state machine to turn its signal face
red or yellow as quickly as possible consistent with safety, and then flash
its signal face either red or yellow.

If the traffic control signal detects a malfunction within itself
it switches itself to Flashing mode.
In addition, if the computer in the traffic
control signal fails there is hardware in the traffic control signal
to flash the lights independent of the computer,
...
the central controller will use the Manual Green sensor to clear paths
